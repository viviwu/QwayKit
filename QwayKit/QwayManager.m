...
static QwayManager *theQwayManager = nil;

NSString *const LINPHONERC_APPLICATION_KEY = @"app";

NSString *const kQwayCoreUpdate = @"LinphoneCoreUpdate";
NSString *const kQwayDisplayStatusUpdate = @"LinphoneDisplayStatusUpdate";
NSString *const kQwayMessageReceived = @"LinphoneMessageReceived";
NSString *const kQwayTextComposeEvent = @"LinphoneTextComposeStarted";
NSString *const kQwayCallUpdate = @"LinphoneCallUpdate";
NSString *const kQwayRegistrationUpdate = @"LinphoneRegistrationUpdate";
NSString *const kQwayAddressBookUpdate = @"LinphoneAddressBookUpdate";
NSString *const kQwayMainViewChange = @"LinphoneMainViewChange";
NSString *const kQwayLogsUpdate = @"LinphoneLogsUpdate";
NSString *const kQwaySettingsUpdate = @"LinphoneSettingsUpdate";
NSString *const kQwayBluetoothAvailabilityUpdate = @"LinphoneBluetoothAvailabilityUpdate";
NSString *const kQwayConfiguringStateUpdate = @"LinphoneConfiguringStateUpdate";
NSString *const kQwayGlobalStateUpdate = @"LinphoneGlobalStateUpdate";
NSString *const kQwayNotifyReceived = @"LinphoneNotifyReceived";
NSString *const kQwayCallEncryptionChanged = @"LinphoneCallEncryptionChanged";
NSString *const kQwayFileTransferSendUpdate = @"LinphoneFileTransferSendUpdate";
NSString *const kQwayFileTransferRecvUpdate = @"LinphoneFileTransferRecvUpdate";

const int kQwayAudioVbrCodecDefaultBitrate = 36; /*you can override this from linphonerc or linphonerc-factory*/
...
NSString *const kQwayOldChatDBFilename = @"chat_database.sqlite";
NSString *const kQwayInternalChatDBFilename = @"linphone_chats.db";
...
+ (QwayManager *)instance {
    @synchronized(self) {
        if (theQwayManager == nil) {
            theQwayManager = [[QwayManager alloc] init];
        }
    }
    return theQwayManager;
}
...
- (id)init {
    if ((self = [super init])) {
        ...
        _isTesting = [QwayManager isRunningTests];
...
- (BOOL)migrateChatDBIfNeeded:(LinphoneCore *)lc {
    ...
    char *errMsg;
    NSError *error;
    NSString *oldDbPath = [QwayManager documentFile:kQwayOldChatDBFilename];
    NSString *newDbPath = [QwayManager documentFile:kQwayInternalChatDBFilename];
...
- (void)migrationLinphoneSettings {
    ...
    // state
    NSString *chatDBFileName = [QwayManager documentFile:kQwayInternalChatDBFilename];
...
static void migrateWizardToAssistant(const char *entry, void *user_data) {
    QwayManager *thiz = (__bridge QwayManager *)(user_data);
...
+ (void)dumpLcConfig {
    if (theLinphoneCore) {
        LpConfig *conf = QwayManager.instance.configDb;
...
- (void)displayStatus:(NSString *)message {
    // Post event
    [NSNotificationCenter.defaultCenter postNotificationName:kQwayDisplayStatusUpdate
                                                      object:self
                                                    userInfo:@{
                                                               @"message" : message
                                                               }];
...
static void linphone_iphone_display_status(struct _LinphoneCore *lc, const char *message) {
    ...
    [(__bridge QwayManager *)linphone_core_get_user_data(lc) displayStatus:status];
}
...
#pragma deploymate pop
    ...
    if (state == LinphoneCallIncomingReceived) {
        ...
        if ([UIApplication sharedApplication].applicationState == UIApplicationStateBackground) {
            ...
            if (![QwayManager.instance popPushCallID:callId]) {
                ...
                if (data->notification) {
                    ...
                    if ([[UIDevice currentDevice].systemVersion floatValue] >= 8 &&
                        ...
                        NSString *ring =
                        ([QwayManager bundleFile:[self lpConfigStringForKey:@"local_ring" inSection:@"sound"].lastPathComponent]
                         ?: [QwayManager bundleFile:@"notes_of_the_optimistic.caf"])
                        .lastPathComponent;
    ...
    [NSNotificationCenter.defaultCenter postNotificationName:kQwayCallUpdate object:self userInfo:dict];
}

static void linphone_iphone_call_state(LinphoneCore *lc, LinphoneCall *call, LinphoneCallState state,
                                       const char *message) {
    [(__bridge QwayManager *)linphone_core_get_user_data(lc) onCall:call StateChanged:state withMessage:message];
}
...
static void linphone_iphone_global_state_changed(LinphoneCore *lc, LinphoneGlobalState gstate, const char *message) {
    [(__bridge QwayManager *)linphone_core_get_user_data(lc) onGlobalStateChanged:gstate withMessage:message];
}
...
- (void)onGlobalStateChanged:(LinphoneGlobalState)state withMessage:(const char *)message {
    ...
    dispatch_async(dispatch_get_main_queue(), ^(void) {
        [NSNotificationCenter.defaultCenter postNotificationName:kQwayGlobalStateUpdate object:self userInfo:dict];
    });
}
...
static void linphone_iphone_configuring_status_changed(LinphoneCore *lc, LinphoneConfiguringState status,
                                                       const char *message) {
    [(__bridge QwayManager *)linphone_core_get_user_data(lc) onConfiguringStatusChanged:status withMessage:message];
}
...
- (void)onConfiguringStatusChanged:(LinphoneConfiguringState)status withMessage:(const char *)message {
    ...
    // dispatch the notification asynchronously
    dispatch_async(dispatch_get_main_queue(), ^(void) {
        [NSNotificationCenter.defaultCenter postNotificationName:kQwayConfiguringStateUpdate
                                                          object:self
                                                        userInfo:dict];
    });
}
...
- (void)onRegister:(LinphoneCore *)lc
    ...
    [NSNotificationCenter.defaultCenter postNotificationName:kQwayRegistrationUpdate object:self userInfo:dict];
}
...
static void linphone_iphone_registration_state(LinphoneCore *lc, LinphoneProxyConfig *cfg,
    ...
    [(__bridge QwayManager *)linphone_core_get_user_data(lc) onRegister:lc cfg:cfg state:state message:message];
}
...
//    NSString *domain = [NSString stringWithUTF8String:domainC];
    NSLog(@"%@ :%@", NSLocalizedString(@"Authentication needed.", nil),
          [NSString  stringWithFormat:NSLocalizedString(@"Registration failed because authentication is "
...
static void linphone_iphone_call_encryption_changed(LinphoneCore *lc, LinphoneCall *call, bool_t on,
    ...
    [(__bridge QwayManager *)linphone_core_get_user_data(lc) onCallEncryptionChanged:lc
                                                                                    call:call
                                                                                      on:on
...
- (void)onCallEncryptionChanged:(LinphoneCore *)lc
    ...
    [NSNotificationCenter.defaultCenter postNotificationName:kQwayCallEncryptionChanged object:self userInfo:dict];
}
...
static void networkReachabilityNotification(CFNotificationCenterRef center, void *observer, CFStringRef name,
    ...
    QwayManager *mgr = QwayManager.instance;
    ...
    NSString *newSSID = [QwayManager getCurrentWifiSSID];
...
void networkReachabilityCallBack(SCNetworkReachabilityRef target, SCNetworkReachabilityFlags flags, void *nilCtx) {
    ...
    QwayManager *lm = QwayManager.instance;
    ...
    if (theLinphoneCore != nil) {
        ...
        if ((flags == 0) || (flags & networkDownFlags)) {
            ...
            [QwayManager kickOffNetworkConnection];
        } else {
...
- (void)setupNetworkReachabilityCallback {
    ...
    _SSID = [QwayManager getCurrentWifiSSID];
...
- (void)finishCoreConfiguration {
    ...
    NSString *zrtpSecretsFileName = [QwayManager documentFile:@"zrtp_secrets"];
    NSString *chatDBFileName = [QwayManager documentFile:kQwayInternalChatDBFilename];
    ...
    NSString *device = [[NSMutableString alloc]
                        initWithString:[NSString
                                        ...
                                        [QwayUtils deviceModelIdentifier],
                                        UIDevice.currentDevice.systemVersion]];
...
//    }
    ...
    NSString *path = [QwayManager bundleFile:@"nowebcamCIF.jpg"];
    if (path) {
    ...
    [NSNotificationCenter.defaultCenter postNotificationName:kQwayCoreUpdate
                                                      object:QwayManager.instance
                                                    userInfo:dict];
}
...
- (void)createLinphoneCore {
    ...
    [QwayUtils enableLogs:[self lpConfigIntForKey:@"debugenable_preference"]];
    connectivity = none;
    ...
    .lastPathComponent;
    NSString *ringback =
    ([QwayManager bundleFile:[self lpConfigStringForKey:@"remote_ring" inSection:@"sound"].lastPathComponent]
     ?: [QwayManager bundleFile:@"ringback.wav"])
    .lastPathComponent;
    NSString *hold =
    ([QwayManager bundleFile:[self lpConfigStringForKey:@"hold_music" inSection:@"sound"].lastPathComponent]
     ?: [QwayManager bundleFile:@"hold.mkv"])
    .lastPathComponent;
    [self lpConfigSetString:[QwayManager bundleFile:ring] forKey:@"local_ring" inSection:@"sound"];
    [self lpConfigSetString:[QwayManager bundleFile:ringback] forKey:@"remote_ring" inSection:@"sound"];
    [self lpConfigSetString:[QwayManager bundleFile:hold] forKey:@"hold_music" inSection:@"sound"];
    ...
    linphone_core_set_root_ca(theLinphoneCore, [QwayManager bundleFile:@"rootca.pem"].UTF8String);
    linphone_core_set_user_certificates_path(theLinphoneCore, [QwayManager cacheDirectory].UTF8String);
...
- (void)playMessageSound {
    ...
    AudioServicesPlaySystemSound(QwayManager.instance.sounds.vibrate);
}
...
- (BOOL)enterBackgroundMode {
    ...
    if (proxyCfg) {
        ...
        if ([QwayManager.instance lpConfigBoolForKey:@"backgroundmode_preference"] || pushNotifEnabled) {
            // For registration register
            [self refreshRegisters];
        }
        
        if ([QwayManager.instance lpConfigBoolForKey:@"backgroundmode_preference"]) {
            // register keepalive
...
- (void)renameDefaultSettings {
    ...
    NSString *src = [QwayManager documentFile:@".linphonerc"];
    NSString *dst = [QwayManager documentFile:@"linphonerc"];
...
- (void)copyDefaultSettings {
    NSString *src = [QwayManager bundleFile:@"linphonerc"];
    NSString *srcIpad = [QwayManager bundleFile:@"linphonerc~ipad"];
    if (IPAD && [[NSFileManager defaultManager] fileExistsAtPath:srcIpad]) {
        src = srcIpad;
    }
    NSString *dst = [QwayManager documentFile:@"linphonerc"];
    [QwayManager copyFile:src destination:dst override:FALSE];
}

- (void)overrideDefaultSettings {
    NSString *factory = [QwayManager bundleFile:@"linphonerc-factory"];
    NSString *factoryIpad = [QwayManager bundleFile:@"linphonerc-factory~ipad"];
    if (IPAD && [[NSFileManager defaultManager] fileExistsAtPath:factoryIpad]) {
        factory = factoryIpad;
    }
    NSString *confiFileName = [QwayManager documentFile:@"linphonerc"];
...
- (void)audioRouteChangeListenerCallback:(NSNotification *)notif {
    ...
    if (!status && newRouteSize > 0) {
        ...
        [NSNotificationCenter.defaultCenter postNotificationName:kQwayBluetoothAvailabilityUpdate
                                                          object:self
                                                        userInfo:dict];
        CFRelease(newRoute);
    }
...
- (BOOL)call:(const LinphoneAddress *)iaddr {
    ...
    if ([QwayManager.instance lpConfigBoolForKey:@"override_domain_with_default_one"]) {
        linphone_address_set_domain(
                                    addr, [[QwayManager.instance lpConfigStringForKey:@"domain" inSection:@"assistant"] UTF8String]);
    }
    
    if (QwayManager.instance.nextCallIsTransfer) {
        ...
        QwayManager.instance.nextCallIsTransfer = NO;
        ms_free(caddr);
    } else {
...
- (void)configureVbrCodecs {
    ...
    int bitrate = lp_config_get_int(
                                    ...
                                    kQwayAudioVbrCodecDefaultBitrate); /*default value is in linphonerc or linphonerc-factory*/
...
- (void)setupGSMInteraction {
    ...
    __block __weak QwayManager *weakSelf = self;
